...
class MultiIndicatorFrameworkTests(unittest.TestCase):
    ...
    def test_registry_runs_only_real_enabled_configuration(self):
        rows=enabled_indicators(self.conn)
        self.assertEqual([r['indicator_key'] for r in rows],['investments_fixed_assets'])
        inv=rows[0]
        self.assertEqual((inv['indicator_id'],inv['period_id']),(701827,8))
        self.assertEqual([d['key'] for d in inv['dimensions']],['kato','krp','sif','gsvziok'])
        with self.conn.cursor() as cur:
            cur.execute("""SELECT indicator_key,indicator_id,period_id,dimensions,roots FROM taldau.metadata_indicator_registry
                WHERE indicator_key<>'investments_fixed_assets' ORDER BY indicator_key""")
            disabled=cur.fetchall()
        self.assertEqual(len(disabled),7)
        self.assertTrue(all(row[1:] == (None,None,None,None) for row in disabled))
    ...
    def test_frozen_config_and_scope_are_immutable(self):
        ...
        with self.conn.cursor() as cur:
            cur.execute("SELECT source_config FROM taldau.bronze_snapshots WHERE batch_id=%s",(batch,))
