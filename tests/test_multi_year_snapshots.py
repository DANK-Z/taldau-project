...
class MultiYearTests(unittest.TestCase):
    ...
    def test_multi_year_reuse_preserves_raw_and_source_and_partial_year(self):
        ...
        with self.conn.cursor() as cur:
            cur.execute('SELECT count(*) FROM bronze.taldau_api_raw')
        ...
        with self.conn.cursor() as cur:
            ...
            cur.execute('''SELECT count(DISTINCT right(period_code,4)) FROM staging.inv_year_cells
                WHERE snapshot_id=%s GROUP BY raw_id,node_ordinal ORDER BY 1 DESC LIMIT 1''',(target,))
            self.assertEqual(cur.fetchone()[0],4)
            cur.execute('''SELECT count(*) FROM bronze.inv_request_tasks q JOIN bronze.taldau_api_raw r ON r.id=q.raw_id
    ...
    def test_missing_raw_only_reports_frontier_and_cannot_call_http(self):
        target=self.prepare_range()
        with self.conn.cursor() as cur:
            cur.execute('''DELETE FROM bronze.inv_reuse_raw x USING bronze.taldau_api_raw r
    ...
    def test_multi_year_publish_matches_gold_idempotently_and_preserves_outside_scope(self):
        ...
        with self.conn.cursor() as cur:
            cur.execute('SELECT row_to_json(v)::text FROM silver.inv_fixed_assets v ORDER BY 1')
            old_silver=cur.fetchall()
            cur.execute('SELECT row_to_json(v)::text FROM gold.fact_inv_fixed_assets v ORDER BY 1')
            old_gold=cur.fetchall()
        for _ in range(2):
            self.assertEqual(publish_snapshot(self.conn,target),expected)
            with self.conn.cursor() as cur:
                cur.execute('SELECT row_to_json(v)::text FROM silver.inv_fixed_assets v WHERE extract(year FROM period_date)=2025 ORDER BY 1')
                self.assertEqual(cur.fetchall(),old_silver)
                cur.execute('SELECT row_to_json(v)::text FROM gold.fact_inv_fixed_assets v WHERE reporting_period=1069 ORDER BY 1')
                ...
                cur.execute('''SELECT indicator_id,reporting_period,kato_id,krp_id,sif_id,gsvziok_id,value
                    FROM silver.inv_fixed_assets WHERE source_snapshot_id=%s ORDER BY 1,2,3,4,5,6''',(target,))
                silver=cur.fetchall()
                cur.execute('''SELECT indicator_id,reporting_period,kato_id,krp_id,sif_id,gsvziok_id,value
                    FROM gold.v_inv_fixed_assets WHERE extract(year FROM start_date) BETWEEN 2023 AND 2024 ORDER BY 1,2,3,4,5,6''')
    ...
    def test_duplicates_and_period_id_conflicts_across_years_block_publish(self):
        ...
        reuse_offline(self.conn,target)
        with self.conn.cursor() as cur:
            cur.execute('''UPDATE staging.inv_year_cells a SET reporting_period=b.reporting_period
                FROM staging.inv_year_cells b WHERE a.snapshot_id=%s AND b.snapshot_id=a.snapshot_id
        ...
        with self.conn.cursor() as cur:
            cur.execute("SELECT check_name FROM quality.inv_snapshot_checks WHERE snapshot_id=%s AND violations>0",(target,))
    ...
    def test_old_snapshot_cannot_overwrite_newer_data_in_overlapping_year(self):
        ...
        reuse_offline(self.conn,target)
        with self.conn.cursor() as cur:
            cur.execute("UPDATE bronze.inv_snapshots SET created_at=now()-interval '10 years' WHERE snapshot_id=%s",(target,))
        with self.assertRaisesRegex(psycopg2.Error,'Newer data is already published'):
            publish_snapshot(self.conn,target)
        with self.conn.cursor() as cur:
            cur.execute('SELECT count(*) FROM silver.inv_fixed_assets WHERE source_snapshot_id=%s',(target,))
    ...
    def test_legacy_null_range_columns_still_publish_2025(self):
        self.complete()
        with self.conn.cursor() as cur:
            cur.execute('UPDATE bronze.inv_snapshots SET year_start=NULL,year_end=NULL WHERE snapshot_id=%s',(self.sid,))
    ...
    def test_new_snapshot_name_cannot_make_stale_reused_raw_fresh(self):
        ...
        with self.conn.cursor() as cur:
            cur.execute('''INSERT INTO bronze.extraction_runs(run_id,pipeline_id,config,scope,status,started_at)
                SELECT %s,pipeline_id,config,scope,'bronze_complete',now()+interval '1 hour'
                FROM bronze.extraction_runs WHERE run_id=%s''',(fresh,self.chunks[268012][1]))
            cur.execute('''INSERT INTO bronze.taldau_api_raw
                ...
                FROM bronze.taldau_api_raw WHERE run_id=%s''',(fresh,self.chunks[268012][1]))
            cur.execute('''UPDATE silver.inv_fixed_assets v SET source_run_id=%s,source_raw_id=new.id
                FROM bronze.taldau_api_raw old,bronze.taldau_api_raw new
            ...
            cur.execute("UPDATE bronze.inv_snapshots SET created_at=now()+interval '2 hours' WHERE snapshot_id=%s",(target,))
        with self.assertRaisesRegex(psycopg2.Error,'Newer data is already published'):
            publish_snapshot(self.conn,target)
    ...
    def test_period_id_cannot_move_existing_gold_to_another_year(self):
        ...
        reuse_offline(self.conn,target)
        with self.conn.cursor() as cur:
            cur.execute("UPDATE staging.inv_year_cells SET reporting_period=1069 WHERE snapshot_id=%s AND period_code='122023'",(target,))
            cur.execute('SELECT row_to_json(v)::text FROM gold.fact_inv_fixed_assets v ORDER BY 1')
            before=cur.fetchall()
        ...
        with self.conn.cursor() as cur:
            cur.execute('''UPDATE silver.inv_fixed_assets SET kato_id=741880
