...
def read_snapshot(conn: Any, snapshot_id: str) -> dict:
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute('SELECT * FROM bronze.inv_snapshots WHERE snapshot_id=%s', (snapshot_id,))
        row = cur.fetchone()
    if row is None:
    ...
    with conn:
        with conn.cursor() as cur:
            ...
            cur.execute('SELECT coalesce(year_start,year),coalesce(year_end,year),reuse_snapshot_id '
                        'FROM bronze.inv_snapshots WHERE snapshot_id=%s', (snapshot_id,))
            ...
            if reuse_snapshot_id:
                cur.execute('SELECT config,state FROM bronze.inv_snapshots WHERE snapshot_id=%s FOR SHARE',
                            (reuse_snapshot_id,))
                source = cur.fetchone()
            ...
            if reuse_snapshot_id:
                cur.execute("""SELECT request_hash FROM bronze.inv_run_raw WHERE run_id IN (
                    SELECT discovery_run_id FROM bronze.inv_snapshots WHERE snapshot_id=%s
                ...
                cur.execute("""INSERT INTO bronze.inv_reuse_raw(snapshot_id,request_hash,raw_id)
                    SELECT %s,request_hash,min(id) FROM bronze.inv_run_raw WHERE run_id IN (
                        SELECT discovery_run_id FROM bronze.inv_snapshots WHERE snapshot_id=%s
                        UNION ALL SELECT run_id FROM bronze.inv_chunks WHERE snapshot_id=%s)
...
class TrackedLoader(BronzeLoader):
    ...
    def fetch(self, terms, dimension, parent='', depth=0):
        ...
        with self.conn:
            with self.conn.cursor() as cur:
                if self.chunk_id is not None:
                    cur.execute('''UPDATE bronze.inv_chunks SET heartbeat_at=clock_timestamp(),
                ...
                cur.execute('''INSERT INTO bronze.inv_request_tasks(run_id,request_hash,state,dimension,request_params)
        ...
        with self.conn.cursor() as cur:
            cur.execute("""SELECT r.id,r.response_text FROM bronze.taldau_api_raw r
                WHERE r.request_hash=%s AND r.endpoint=%s AND r.request_params=%s
                  ...
                  AND (r.run_id=%s OR r.id=(SELECT raw_id FROM bronze.inv_request_tasks
                       WHERE run_id=%s AND request_hash=%s AND state='complete')
                       OR r.id=(SELECT raw_id FROM bronze.inv_reuse_raw WHERE request_hash=%s
                            AND snapshot_id=(SELECT scope->>'snapshot_id' FROM bronze.extraction_runs WHERE run_id=%s)))
                ORDER BY CASE WHEN r.id=(SELECT raw_id FROM bronze.inv_request_tasks
        ...
        if cached:
            ...
            with self.conn.cursor() as cur:
                cur.execute('SELECT id FROM bronze.taldau_api_raw WHERE run_id=%s AND request_hash=%s',
                            (self.run_id,digest))
                raw_id = cur.fetchone()[0]
        with self.conn:
            with self.conn.cursor() as cur:
                cur.execute("""UPDATE bronze.inv_request_tasks SET state='complete',raw_id=%s,completed_at=now()
...
def discover_territories(conn: Any, snapshot_id: str, *, allow_http: bool = True) -> dict:
    ...
    try:
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute("UPDATE bronze.inv_snapshots SET state='discovering',last_error=NULL WHERE snapshot_id=%s", (snapshot_id,))
                cur.execute("UPDATE bronze.extraction_runs SET status='loading',last_error=NULL WHERE run_id=%s", (run_id,))
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute('SELECT bronze.stage_inv_run(%s,%s)', (snapshot_id,run_id))
                cur.execute('SELECT quality.inv_run_coverage(%s)', (run_id,))
                errors = cur.fetchone()[0]
                if errors:
                    raise ValueError(f'Incomplete KATO traversal: {errors}')
                cur.execute('SELECT quality.inv_discovery_errors(%s)', (snapshot_id,))
                if cur.fetchone()[0]:
                    raise ValueError('Invalid or conflicting KATO hierarchy; inspect saved raw responses')
                cur.execute("UPDATE bronze.extraction_runs SET status='bronze_complete',completed_at=now() WHERE run_id=%s", (run_id,))
                ...
                cur.execute('''INSERT INTO bronze.extraction_runs(run_id,pipeline_id,config,scope)
                    ...
                    FROM bronze.inv_snapshot_members WHERE run_id=%s AND dimension='kato'
                ...
                cur.execute('''UPDATE bronze.inv_snapshots SET discovery_complete=true,state='loading',
                    expected_chunks=(SELECT count(*) FROM bronze.inv_chunks WHERE snapshot_id=%s)
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute("UPDATE bronze.inv_snapshots SET state='failed',last_error=%s WHERE snapshot_id=%s", (str(exc)[:4000],snapshot_id))
        raise
    finally:
        if loader:
...
def pending_chunk_ids(conn: Any, snapshot_id: str, limit: int = WAVE_SIZE) -> list[int]:
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute('''SELECT discovery_complete,state FROM bronze.inv_snapshots WHERE snapshot_id=%s FOR UPDATE''', (snapshot_id,))
            row = cur.fetchone()
            ...
            cur.execute('''SELECT count(*) FROM bronze.inv_chunks WHERE snapshot_id=%s
            ...
            cur.execute("UPDATE bronze.inv_snapshots SET state='loading',last_error=NULL WHERE snapshot_id=%s", (snapshot_id,))
            cur.execute('''SELECT chunk_id FROM bronze.inv_chunks WHERE snapshot_id=%s AND state<>'complete'
...
def extract_chunk(conn: Any, chunk_id: int, *, allow_http: bool = True) -> dict:
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute('SELECT * FROM bronze.inv_chunks WHERE chunk_id=%s', (chunk_id,))
        chunk = cur.fetchone()
    if not chunk:
    ...
    try:
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute('SELECT state,lease_until FROM bronze.inv_chunks WHERE chunk_id=%s FOR UPDATE', (chunk_id,))
                ...
                cur.execute('''UPDATE bronze.inv_chunks SET state='running',attempt=attempt+1,
        ...
        with conn:
            with conn.cursor() as cur:
                ...
                cur.execute("UPDATE bronze.extraction_runs SET status='bronze_complete',completed_at=now() WHERE run_id=%s", (run_id,))
                cur.execute('SELECT staging.stage_inv_chunk(%s)', (chunk_id,))
                staged = cur.fetchone()[0]
                cur.execute('''UPDATE bronze.inv_chunks SET state='complete',completed_at=now(),lease_until=NULL,
                    raw_count=(SELECT count(*) FROM bronze.inv_run_raw WHERE run_id=%s)
        ...
        if claimed:
            with conn:
                with conn.cursor() as cur:
                    cur.execute('''UPDATE bronze.inv_chunks SET state='failed',last_error=%s,lease_until=NULL
                        WHERE chunk_id=%s AND lease_token=%s''', (str(exc)[:4000],chunk_id,token))
                    cur.execute("UPDATE bronze.extraction_runs SET status='failed',last_error=%s WHERE run_id=%s", (str(exc)[:4000],run_id))
...
def wave_outcome(conn: Any, snapshot_id: str, selected_ids: list[int]) -> str:
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute('''SELECT state,count(*) FROM bronze.inv_chunks WHERE snapshot_id=%s GROUP BY state''', (snapshot_id,))
            ...
            if counts.get('failed',0) or counts.get('running',0) or selected_complete!=len(selected_ids):
                cur.execute("UPDATE bronze.inv_snapshots SET state='failed',last_error=%s WHERE snapshot_id=%s",
...
def validate_snapshot(conn: Any, snapshot_id: str) -> dict:
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute('SELECT quality.validate_inv_snapshot(%s)', (snapshot_id,))
            result = cur.fetchone()[0]
    if not result['valid']:
        raise ValueError(f'Snapshot validation failed: {result}; see quality.inv_snapshot_checks')
    return result
...
def publish_snapshot(conn: Any, snapshot_id: str) -> int:
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute('SELECT silver.publish_inv_snapshot(%s)', (snapshot_id,))
            silver_rows = cur.fetchone()[0]
            cur.execute('SELECT gold.publish_inv_snapshot(%s)', (snapshot_id,))
