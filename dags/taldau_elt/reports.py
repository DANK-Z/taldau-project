...
def snapshot_summary(conn: Any, snapshot_id: str) -> dict:
    ...
    with conn.cursor() as cur:
        cur.execute('''WITH s AS (
            SELECT * FROM taldau.bronze_inv_snapshots WHERE snapshot_id=%s
        ), chunks AS MATERIALIZED (
            SELECT c.* FROM taldau.bronze_inv_chunks c JOIN s USING(snapshot_id)
            ...
            SELECT v.*,
                ((value IS NULL AND raw_value IS DISTINCT FROM 'x')
                  ...
                  OR (period_code !~ '^(0[1-9]|1[0-2])[0-9]{4}$' OR taldau.bronze_inv_int(right(period_code,4)) NOT BETWEEN coalesce(s.year_start,s.year) AND coalesce(s.year_end,s.year))
            ...
            FROM taldau.staging_inv_year_cells v JOIN s USING(snapshot_id) JOIN chunks c USING(chunk_id)
        ), duplicates AS (
            ...
            SELECT taldau.bronze_inv_int(right(coalesce(a.period_code,e.period_code),4)) AS year,
            ...
            FROM actual_months a FULL JOIN (
                SELECT e.* FROM taldau.quality_inv_month_expectations e,s WHERE e.year BETWEEN coalesce(s.year_start,s.year) AND coalesce(s.year_end,s.year)
            ) e ON a.period_code=e.period_code AND a.reporting_period=e.reporting_period
        ), checks AS (
            SELECT q.* FROM taldau.quality_inv_snapshot_checks q JOIN s USING(snapshot_id)
        ), empty_chunks AS (
        ...
        SELECT jsonb_build_object(
            ...
            'years',(SELECT jsonb_agg(to_jsonb(y) ORDER BY year) FROM taldau.quality_inv_year_coverage y WHERE y.snapshot_id=s.snapshot_id),
            ...
            'territories_total',(SELECT count(DISTINCT member_id) FROM taldau.bronze_inv_snapshot_members
            ...
            'raw_responses',(SELECT count(DISTINCT id) FROM taldau.bronze_inv_run_raw WHERE run_id=s.discovery_run_id
            ...
            'expected_ets_rows',(SELECT sum(expected_rows) FROM taldau.quality_inv_month_expectations WHERE year BETWEEN coalesce(s.year_start,s.year) AND coalesce(s.year_end,s.year)),
