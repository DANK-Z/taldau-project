...
CREATE TABLE IF NOT EXISTS reconciliation.ets_inv_datasets (
...
CREATE TABLE IF NOT EXISTS reconciliation.ets_inv_raw (
    row_id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    dataset_id text NOT NULL REFERENCES reconciliation.ets_inv_datasets,
    source_row jsonb NOT NULL
);
CREATE INDEX IF NOT EXISTS ets_inv_dataset_idx ON reconciliation.ets_inv_raw(dataset_id);
CREATE TABLE IF NOT EXISTS reconciliation.ets_inv_key_map (
    dataset_id text NOT NULL REFERENCES reconciliation.ets_inv_datasets,
...
COMMENT ON TABLE reconciliation.ets_inv_key_map IS
...
CREATE OR REPLACE VIEW reconciliation.v_ets_inv_resolved AS
SELECT e.dataset_id,e.row_id,701827::bigint AS indicator_id,
    bronze.inv_int(e.source_row->>'reporting_period') AS reporting_period,
    ...
    e.source_row
FROM reconciliation.ets_inv_raw e
LEFT JOIN reconciliation.ets_inv_key_map k ON k.dataset_id=e.dataset_id AND k.dimension='kato' AND k.ets_key=e.source_row->>'kato1'
LEFT JOIN reconciliation.ets_inv_key_map r ON r.dataset_id=e.dataset_id AND r.dimension='krp' AND r.ets_key=e.source_row->>'krp'
LEFT JOIN reconciliation.ets_inv_key_map s ON s.dataset_id=e.dataset_id AND s.dimension='sif' AND s.ets_key=e.source_row->>'sif'
LEFT JOIN reconciliation.ets_inv_key_map g ON g.dataset_id=e.dataset_id AND g.dimension='gsvziok' AND g.ets_key=e.source_row->>'gsvziok';

CREATE TABLE IF NOT EXISTS reconciliation.inv_results (
...
CREATE TABLE IF NOT EXISTS reconciliation.inv_differences (
    snapshot_id text NOT NULL,dataset_id text NOT NULL,reporting_period bigint NOT NULL,
...
BEGIN
    ...
    WITH d AS (
        ...
        FROM silver.inv_fixed_assets WHERE source_snapshot_id=p_snapshot GROUP BY 1,2,3,4,5,6
    ), e AS (
        SELECT indicator_id,reporting_period,kato_id,krp_id,sif_id,gsvziok_id,count(*) AS n,min(value) AS value
