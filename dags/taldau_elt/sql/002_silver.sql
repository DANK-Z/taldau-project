...
CREATE OR REPLACE VIEW taldau.bronze_v_taldau_tree_nodes AS
...
CREATE OR REPLACE VIEW taldau.bronze_v_inv_candidates AS
...
FROM taldau.bronze_v_taldau_tree_nodes n
JOIN taldau.bronze_extraction_runs r USING(run_id)
WHERE n.dimension='gsvziok' AND n.node ? (r.scope->>'period_code');

CREATE TABLE IF NOT EXISTS taldau.silver_dim_territory (
...
CREATE TABLE IF NOT EXISTS taldau.silver_inv_fixed_assets (
    indicator_id bigint NOT NULL,
    kato_id bigint NOT NULL REFERENCES taldau.silver_dim_territory,
...
CREATE INDEX IF NOT EXISTS silver_inv_fixed_assets_run_idx ON taldau.silver_inv_fixed_assets(source_run_id);

CREATE OR REPLACE FUNCTION taldau.bronze_validate_inv_pilot(p_run text) RETURNS jsonb
LANGUAGE plpgsql AS $$
DECLARE r taldau.bronze_extraction_runs; total_rows bigint; numeric_rows bigint; x_rows bigint;
...
BEGIN
    ...
    FROM taldau.bronze_v_inv_candidates WHERE run_id=p_run;
    ...
    IF EXISTS (
        SELECT dimension,member_id FROM taldau.bronze_v_taldau_tree_nodes WHERE run_id=p_run
    ...
    IF EXISTS (
        SELECT 1 FROM taldau.bronze_v_taldau_tree_nodes
...
CREATE OR REPLACE FUNCTION taldau.silver_refresh_inv_pilot(p_run text) RETURNS jsonb
LANGUAGE plpgsql AS $$
DECLARE r taldau.bronze_extraction_runs; result jsonb;
BEGIN
    PERFORM pg_advisory_xact_lock(hashtextextended('taldau.silver_inv_fixed_assets:pilot',0));
    SELECT * INTO STRICT r FROM taldau.bronze_extraction_runs WHERE run_id=p_run;
    result := taldau.bronze_validate_inv_pilot(p_run);
    -- An old replay must not silently overwrite a more recently acquired version.
    IF EXISTS (SELECT 1 FROM taldau.silver_inv_fixed_assets s JOIN taldau.bronze_extraction_runs other
    ...
    FROM taldau.bronze_v_taldau_tree_nodes WHERE run_id=p_run AND dimension='kato'
    ...
    DELETE FROM taldau.silver_inv_fixed_assets WHERE indicator_id=(r.config->>'indicator_id')::bigint
    ...
    FROM taldau.bronze_v_inv_candidates c
    JOIN taldau.silver_dim_territory t ON t.territory_id=c.kato_id
    JOIN (SELECT DISTINCT member_id,member_name FROM taldau.bronze_v_taldau_tree_nodes
    ...
    END IF;
    UPDATE taldau.bronze_extraction_runs SET status='silver_validated' WHERE run_id=p_run;
    RETURN result;
END $$;
