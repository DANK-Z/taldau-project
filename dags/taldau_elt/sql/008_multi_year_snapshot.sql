...
ALTER TABLE bronze.inv_snapshots ADD COLUMN IF NOT EXISTS year_start integer;
ALTER TABLE bronze.inv_snapshots ADD COLUMN IF NOT EXISTS year_end integer;
ALTER TABLE bronze.inv_snapshots ADD COLUMN IF NOT EXISTS reuse_snapshot_id text REFERENCES bronze.inv_snapshots(snapshot_id);
ALTER TABLE bronze.inv_snapshots DROP CONSTRAINT IF EXISTS inv_snapshots_year_check;
DO $$ BEGIN
    IF NOT EXISTS(SELECT 1 FROM pg_constraint WHERE conrelid='bronze.inv_snapshots'::regclass AND conname='inv_snapshot_range_check') THEN
        ALTER TABLE bronze.inv_snapshots ADD CONSTRAINT inv_snapshot_range_check CHECK (
...
CREATE TABLE IF NOT EXISTS bronze.inv_reuse_raw (
...
CREATE INDEX IF NOT EXISTS inv_reuse_raw_id_idx ON bronze.inv_reuse_raw(raw_id);
...
CREATE OR REPLACE VIEW bronze.inv_run_raw AS
...
SELECT r.id,q.run_id,r.indicator_id,r.endpoint,r.period_id,r.request_params,r.request_hash,
       r.response_data,r.response_text,r.response_hash,r.http_status,r.dimension,r.tree_depth,r.loaded_at,
       r.run_id AS source_run_id
FROM bronze.inv_request_tasks q JOIN bronze.taldau_api_raw r ON r.id=q.raw_id
WHERE q.state='complete' AND r.run_id<>q.run_id;

CREATE OR REPLACE VIEW quality.inv_year_coverage AS
...
LEFT JOIN LATERAL (
    ...
    FROM staging.inv_year_cells v WHERE v.snapshot_id=s.snapshot_id
...
BEGIN
    ...
    CROSS JOIN LATERAL (
        ...
        WHERE key ~ '^y?[0-9]{6}$' AND bronze.inv_int(right(key,4)) BETWEEN coalesce(s.year_start,s.year) AND coalesce(s.year_end,s.year)
    ) k
...
r AS MATERIALIZED (SELECT * FROM bronze.inv_run_raw WHERE run_id=p_run),
...
errors AS (
 ...
 WHERE q.run_id=p_run AND (q.state<>'complete' OR r.id IS NULL OR r.run_id<>q.run_id
       ...
       OR (r.source_run_id<>q.run_id AND NOT EXISTS(SELECT 1 FROM bronze.inv_reuse_raw reuse
           WHERE reuse.snapshot_id=(SELECT scope->>'snapshot_id' FROM bronze.extraction_runs WHERE run_id=p_run)
...
BEGIN
    ...
    UNION ALL SELECT p_snapshot,'incomplete_chunks',count(*) FROM bronze.inv_chunks c
        ...
        AND (c.state<>'complete' OR c.staged_at IS NULL OR c.completed_at IS NULL
          OR r.status<>'bronze_complete' OR c.raw_count IS DISTINCT FROM
             (SELECT count(*) FROM bronze.inv_run_raw raw WHERE raw.run_id=c.run_id))
    ...
    UNION ALL SELECT p_snapshot,'invalid_period_codes',count(*) FROM staging.inv_year_cells
        WHERE snapshot_id=p_snapshot AND (period_code !~ '^(0[1-9]|1[0-2])[0-9]{4}$' OR bronze.inv_int(right(period_code,4)) NOT BETWEEN coalesce(s.year_start,s.year) AND coalesce(s.year_end,s.year))
    ...
    UNION ALL SELECT p_snapshot,'missing_dimension_members',count(*) FROM staging.inv_year_cells v
        WHERE v.snapshot_id=p_snapshot AND (
         ...
         OR NOT EXISTS(SELECT 1 FROM bronze.inv_snapshot_members m WHERE m.run_id=v.run_id AND m.dimension='gsvziok' AND m.member_id=v.gsvziok_id));
    ...
    FROM actual a FULL JOIN (SELECT * FROM quality.inv_month_expectations WHERE year BETWEEN coalesce(s.year_start,s.year) AND coalesce(s.year_end,s.year)) e
    ...
    RETURN jsonb_build_object('snapshot_id',p_snapshot,'valid',bad=0,'violations',bad,'numeric_rows',cells,
        'ets_expected',(SELECT sum(expected_rows) FROM quality.inv_month_expectations WHERE year BETWEEN coalesce(s.year_start,s.year) AND coalesce(s.year_end,s.year)),
        'delta',(SELECT sum(delta) FROM quality.inv_month_diagnostics WHERE snapshot_id=p_snapshot));
END $$;
...
BEGIN
    ...
    IF EXISTS(SELECT 1 FROM silver.inv_fixed_assets v JOIN bronze.extraction_runs r ON r.run_id=v.source_run_id
        ...
        AND (r.started_at>s.created_at OR (
            v.source_raw_id NOT IN (SELECT raw.id FROM bronze.inv_run_raw raw
                WHERE raw.run_id=s.discovery_run_id OR raw.run_id IN (SELECT run_id FROM bronze.inv_chunks WHERE snapshot_id=p_snapshot))
            AND r.started_at>(SELECT min(raw.loaded_at) FROM bronze.inv_run_raw raw
                WHERE raw.run_id=s.discovery_run_id OR raw.run_id IN (SELECT run_id FROM bronze.inv_chunks WHERE snapshot_id=p_snapshot)))) AND (v.source_snapshot_id IS DISTINCT FROM p_snapshot)) THEN
...
CREATE OR REPLACE FUNCTION gold.publish_inv_snapshot(p_snapshot text) RETURNS bigint
LANGUAGE plpgsql AS $$
DECLARE s bronze.inv_snapshots; check_result jsonb; silver_rows bigint; inserted bigint;
BEGIN
    ...
    END IF;
    SELECT count(*) INTO silver_rows FROM silver.inv_fixed_assets WHERE source_snapshot_id=p_snapshot;
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM silver.inv_fixed_assets v WHERE v.source_snapshot_id=p_snapshot
    ...
    IF EXISTS (
        SELECT 1 FROM silver.inv_fixed_assets v
        ...
        WHERE v.source_snapshot_id=p_snapshot AND NOT EXISTS (
            SELECT 1 FROM bronze.inv_snapshot_members m WHERE m.snapshot_id=p_snapshot
                AND m.dimension=d.dimension AND m.member_id=d.member_id)
    ) THEN RAISE EXCEPTION 'Missing dimension member in snapshot'; END IF;

    LOCK TABLE gold.dim_inv_member, gold.dim_inv_period, gold.fact_inv_fixed_assets
    ...
    FROM bronze.inv_snapshot_members m WHERE m.snapshot_id=p_snapshot
    ...
    IF EXISTS (
        SELECT 1 FROM silver.inv_fixed_assets v JOIN gold.dim_inv_period p
    ...
    FROM silver.inv_fixed_assets WHERE source_snapshot_id=p_snapshot
    ...
    DELETE FROM gold.fact_inv_fixed_assets f USING gold.dim_inv_period p
    ...
    FROM silver.inv_fixed_assets v
    JOIN gold.dim_inv_member k ON k.indicator_id=v.indicator_id AND k.dimension='kato' AND k.source_id=v.kato_id
    JOIN gold.dim_inv_member r ON r.indicator_id=v.indicator_id AND r.dimension='krp' AND r.source_id=v.krp_id
    JOIN gold.dim_inv_member f ON f.indicator_id=v.indicator_id AND f.dimension='sif' AND f.source_id=v.sif_id
    JOIN gold.dim_inv_member g ON g.indicator_id=v.indicator_id AND g.dimension='gsvziok' AND g.source_id=v.gsvziok_id
    ...
    IF EXISTS (
        (SELECT indicator_id,reporting_period,kato_id,krp_id,sif_id,gsvziok_id,value
         ...
         EXCEPT
         SELECT indicator_id,reporting_period,kato_id,krp_id,sif_id,gsvziok_id,value
         FROM gold.v_inv_fixed_assets WHERE indicator_id=(s.config->>'indicator_id')::bigint
...
CREATE OR REPLACE FUNCTION reconciliation.capture_inv_ets(p_dataset text,p_source regclass,p_year int DEFAULT 2025)
RETURNS bigint LANGUAGE plpgsql AS $$
DECLARE total bigint;
BEGIN
    IF p_year NOT BETWEEN 2023 AND 2026 THEN RAISE EXCEPTION 'Supported baseline years: 2023..2026'; END IF;
    IF EXISTS(SELECT 1 FROM reconciliation.ets_inv_datasets WHERE dataset_id=p_dataset) THEN
        RAISE EXCEPTION 'ETS dataset is immutable; use a new dataset_id for a new snapshot';
    END IF;
    INSERT INTO reconciliation.ets_inv_datasets(dataset_id,source_relation,year)
    ...
    EXECUTE format('INSERT INTO reconciliation.ets_inv_raw(dataset_id,source_row)
    ...
    UPDATE reconciliation.ets_inv_datasets SET row_count=total WHERE dataset_id=p_dataset;
    RETURN total;
END $$;

CREATE OR REPLACE FUNCTION reconciliation.compare_inv_ets(p_snapshot text,p_dataset text) RETURNS bigint
LANGUAGE plpgsql AS $$
DECLARE total bigint;
BEGIN
    IF NOT EXISTS(SELECT 1 FROM bronze.inv_snapshots WHERE snapshot_id=p_snapshot AND state='published') THEN
        RAISE EXCEPTION 'Publish a complete validated snapshot before comparing Silver';
    END IF;
    IF NOT EXISTS(SELECT 1 FROM reconciliation.ets_inv_datasets WHERE dataset_id=p_dataset AND row_count>0) THEN
        RAISE EXCEPTION 'Capture an ETS dataset first';
    END IF;
    IF EXISTS(SELECT 1 FROM reconciliation.v_ets_inv_resolved WHERE dataset_id=p_dataset AND
    ...
    WITH d AS (
        ...
        FROM silver.inv_fixed_assets WHERE source_snapshot_id=p_snapshot
          AND extract(year FROM period_date)=(SELECT year FROM reconciliation.ets_inv_datasets WHERE dataset_id=p_dataset) GROUP BY 1,2,3,4,5,6
    ), e AS (
        SELECT indicator_id,reporting_period,kato_id,krp_id,sif_id,gsvziok_id,count(*) AS n,min(value) AS value
        FROM reconciliation.v_ets_inv_resolved WHERE dataset_id=p_dataset GROUP BY 1,2,3,4,5,6
    )
    SELECT indicator_id,reporting_period,kato_id,krp_id,sif_id,gsvziok_id,
    ...
    DELETE FROM reconciliation.inv_results WHERE snapshot_id=p_snapshot AND dataset_id=p_dataset;
    DELETE FROM reconciliation.inv_differences WHERE snapshot_id=p_snapshot AND dataset_id=p_dataset;
    INSERT INTO reconciliation.inv_results
    ...
    INSERT INTO reconciliation.inv_differences
