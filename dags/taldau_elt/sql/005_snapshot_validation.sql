...
r AS MATERIALIZED (SELECT * FROM taldau.bronze_taldau_api_raw WHERE run_id=p_run),
...
BEGIN
    ...
    UNION ALL SELECT p_snapshot,'incomplete_chunks',count(*) FROM taldau.bronze_inv_chunks c
        ...
        AND (c.state<>'complete' OR c.staged_at IS NULL OR c.completed_at IS NULL
          OR r.status<>'bronze_complete' OR c.raw_count IS DISTINCT FROM
             (SELECT count(*) FROM taldau.bronze_taldau_api_raw raw WHERE raw.run_id=c.run_id))
    ...
    FROM actual a FULL JOIN (SELECT * FROM taldau.quality_inv_month_expectations WHERE year=s.year) e
