...
CREATE TABLE IF NOT EXISTS taldau.bronze_inv_snapshots (
...
CREATE TABLE IF NOT EXISTS taldau.bronze_inv_chunks (
...
CREATE INDEX IF NOT EXISTS bronze_inv_chunks_pending_idx ON taldau.bronze_inv_chunks(snapshot_id,state,chunk_id);

-- Request checkpoint: written before HTTP; completed only after durable raw storage.
CREATE TABLE IF NOT EXISTS taldau.bronze_inv_request_tasks (
...
CREATE TABLE IF NOT EXISTS taldau.bronze_inv_snapshot_members (
    snapshot_id text NOT NULL REFERENCES taldau.bronze_inv_snapshots,
    run_id text NOT NULL REFERENCES taldau.bronze_extraction_runs,
...
CREATE INDEX IF NOT EXISTS bronze_inv_members_lookup_idx
    ON taldau.bronze_inv_snapshot_members(snapshot_id,dimension,member_id);
CREATE INDEX IF NOT EXISTS bronze_inv_members_run_idx ON taldau.bronze_inv_snapshot_members(run_id);
...
CREATE TABLE IF NOT EXISTS taldau.staging_inv_year_cells (
    snapshot_id text NOT NULL REFERENCES taldau.bronze_inv_snapshots,
    chunk_id bigint NOT NULL REFERENCES taldau.bronze_inv_chunks,
    run_id text NOT NULL REFERENCES taldau.bronze_extraction_runs,
    raw_id bigint NOT NULL REFERENCES taldau.bronze_taldau_api_raw,
    node_ordinal bigint NOT NULL,
    indicator_id bigint,
...
CREATE INDEX IF NOT EXISTS staging_inv_cells_chunk_idx ON taldau.staging_inv_year_cells(chunk_id);
CREATE INDEX IF NOT EXISTS staging_inv_cells_grain_idx ON taldau.staging_inv_year_cells
    (snapshot_id,indicator_id,reporting_period,kato_id,krp_id,sif_id,gsvziok_id);

ALTER TABLE taldau.silver_inv_fixed_assets ADD COLUMN IF NOT EXISTS source_snapshot_id text
    REFERENCES taldau.bronze_inv_snapshots;
CREATE INDEX IF NOT EXISTS silver_inv_snapshot_idx ON taldau.silver_inv_fixed_assets(source_snapshot_id);

CREATE TABLE IF NOT EXISTS taldau.quality_inv_month_expectations (
...
INSERT INTO taldau.quality_inv_month_expectations VALUES
...
CREATE TABLE IF NOT EXISTS taldau.quality_inv_snapshot_checks (
    snapshot_id text NOT NULL REFERENCES taldau.bronze_inv_snapshots,
...
CREATE TABLE IF NOT EXISTS taldau.quality_inv_month_diagnostics (
    snapshot_id text NOT NULL REFERENCES taldau.bronze_inv_snapshots,
...
CREATE OR REPLACE FUNCTION taldau.bronze_inv_int(t text) RETURNS bigint
