...
CREATE TABLE IF NOT EXISTS taldau.gold_dim_inv_member (
...
CREATE TABLE IF NOT EXISTS taldau.gold_dim_inv_period (
    indicator_id bigint NOT NULL,
...
CREATE TABLE IF NOT EXISTS taldau.gold_fact_inv_fixed_assets (
    indicator_id bigint NOT NULL,
    reporting_period bigint NOT NULL,
    kato_key bigint NOT NULL REFERENCES taldau.gold_dim_inv_member,
    krp_key bigint NOT NULL REFERENCES taldau.gold_dim_inv_member,
    sif_key bigint NOT NULL REFERENCES taldau.gold_dim_inv_member,
    gsvziok_key bigint NOT NULL REFERENCES taldau.gold_dim_inv_member,
    ...
    FOREIGN KEY(indicator_id,reporting_period) REFERENCES taldau.gold_dim_inv_period
);
CREATE OR REPLACE VIEW taldau.gold_v_inv_fixed_assets AS
...
FROM taldau.gold_fact_inv_fixed_assets f
JOIN taldau.gold_dim_inv_period p USING(indicator_id,reporting_period)
JOIN taldau.gold_dim_inv_member k ON k.member_key=f.kato_key AND k.dimension='kato'
JOIN taldau.gold_dim_inv_member r ON r.member_key=f.krp_key AND r.dimension='krp'
JOIN taldau.gold_dim_inv_member s ON s.member_key=f.sif_key AND s.dimension='sif'
JOIN taldau.gold_dim_inv_member g ON g.member_key=f.gsvziok_key AND g.dimension='gsvziok';

-- Source-provided totals only: no sum over parents/children or cumulative months.
CREATE OR REPLACE VIEW taldau.gold_mart_inv_territory_totals AS
...
FROM taldau.gold_v_inv_fixed_assets WHERE krp_is_total AND sif_is_total AND gsvziok_is_total;

CREATE OR REPLACE FUNCTION taldau.gold_refresh_inv_pilot(p_run text) RETURNS bigint
LANGUAGE plpgsql AS $$
DECLARE r taldau.bronze_extraction_runs; inserted bigint;
BEGIN
    PERFORM pg_advisory_xact_lock(hashtextextended('taldau.silver_inv_fixed_assets:pilot',0));
    SELECT * INTO STRICT r FROM taldau.bronze_extraction_runs WHERE run_id=p_run;
    IF r.status NOT IN ('silver_validated','gold_validated') THEN
        RAISE EXCEPTION 'Silver must be validated before Gold';
    END IF;
    IF (SELECT count(*) FROM taldau.silver_inv_fixed_assets WHERE source_run_id=p_run)<>(r.scope->>'expected_numeric')::bigint THEN
    ...
    FROM taldau.bronze_v_taldau_tree_nodes WHERE run_id=p_run
    ...
    FROM taldau.silver_inv_fixed_assets WHERE source_run_id=p_run
    ...
    DELETE FROM taldau.gold_fact_inv_fixed_assets f USING taldau.gold_dim_inv_member k
    ...
    FROM taldau.silver_inv_fixed_assets s
    JOIN taldau.gold_dim_inv_member k ON k.indicator_id=s.indicator_id AND k.dimension='kato' AND k.source_id=s.kato_id
    JOIN taldau.gold_dim_inv_member rp ON rp.indicator_id=s.indicator_id AND rp.dimension='krp' AND rp.source_id=s.krp_id
    JOIN taldau.gold_dim_inv_member si ON si.indicator_id=s.indicator_id AND si.dimension='sif' AND si.source_id=s.sif_id
    JOIN taldau.gold_dim_inv_member g ON g.indicator_id=s.indicator_id AND g.dimension='gsvziok' AND g.source_id=s.gsvziok_id
    ...
    IF EXISTS (
        (SELECT kato_id,krp_id,sif_id,gsvziok_id,reporting_period,value FROM taldau.silver_inv_fixed_assets WHERE source_run_id=p_run
         EXCEPT SELECT kato_id,krp_id,sif_id,gsvziok_id,reporting_period,value FROM taldau.gold_v_inv_fixed_assets WHERE source_run_id=p_run)
        UNION ALL
        (SELECT kato_id,krp_id,sif_id,gsvziok_id,reporting_period,value FROM taldau.gold_v_inv_fixed_assets WHERE source_run_id=p_run
         EXCEPT SELECT kato_id,krp_id,sif_id,gsvziok_id,reporting_period,value FROM taldau.silver_inv_fixed_assets WHERE source_run_id=p_run)
    ) THEN RAISE EXCEPTION 'Gold/Silver mismatch'; END IF;
    UPDATE taldau.bronze_extraction_runs SET status='gold_validated' WHERE run_id=p_run;
    RETURN inserted;
END $$;
